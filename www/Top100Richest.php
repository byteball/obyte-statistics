...
while( $row = $results->fetchArray(SQLITE3_ASSOC) ){
	$disclaimers .= ($row[ 'address' ] == 'MZ4GUQC7WUKZKKLGAS3H3FSDKLHI7HFO') ? '#'. $i .' <span class="address">'. $row[ 'address' ] .'</span> holds the remaining disribution amount.<br>' : '';
	$disclaimers .= ($row[ 'address' ] == 'QR542JXX7VJ5UJOZDKHTJCXAYWOATID2') ? '#'. $i .' <span class="address">'. $row[ 'address' ] .'</span> is Bittrex exchange wallet.<br>' : '';
	$disclaimers .= ($row[ 'address' ] == 'BZUAVP5O4ND6N3PVEUZJOATXFPIKHPDC') ? '#'. $i .' <span class="address">'. $row[ 'address' ] .'</span> is founder\'s 1% of total supply.<br>' : '';
	$disclaimers .= ($row[ 'address' ] == 'TUOMEGAZPYLZQBJKLEM2BGKYR2Q5SEYS') ? '#'. $i .' <span class="address">'. $row[ 'address' ] .'</span> is distribution that will be given to first 100 million users, 100KB each.<br>' : '';
